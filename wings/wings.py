...
try:
    ...
    def translateIT():
        subprocess.run(["clear"])
        subprocess.run(["figlet", "TRANSLATOR"])

        while True:
            ques = input("Çeviriyi seçtiniz, Devam etmek ister misiniz? e/h >> ")
            if ques.lower() == "e":
                subprocess.run(["clear"])
                subprocess.run(["figlet", "TRANSLATOR"])

                translator = Translator()
                translatea = input("çevirilecek metni girin >> ")
                translated = translator.translate(translatea)
    ...
    def covid19():
        ...
        while True:
            ques = input("\nCovid19 İnfo seçtiniz devam etmek ister misiniz? e/h >> ")
            if ques.lower() == "e":
                subprocess.call(["clear"])
                print(
                    """
                1)Ülke Kodlarını Görüntüle(uygulamada ülke kodu istenecek istediğiniz ülkenin kodunda buradan bakın)
                2)Uygulamayı Başlat
                """
                )
                ...
                if sec == 1:
                    subprocess.run(["clear"])
                    coviddd = covid.list_countries()
                    print(coviddd)

                elif sec == 2:
                    subprocess.call(["clear"])
                    print(
                        "Dünyada ki aktif hasta sayısı: ",
                        covid.get_total_active_cases(),
                    )
                    print(
                        "Dünyada ki toplam vaka sayısı: ", covid.get_total_recovered()
                    )
                    print("Dünyada toplam ölüm sayısı : ", covid.get_total_deaths())

                    code = input("\n\nÜlke Kodunu girin |örn: us| >> ")
    ...
    def qr_term():
        subprocess.run("clear")
        subprocess.run(["figlet","QR_CODE"])

        while True:
            ques = input("QR kod'u seçtiniz devam etmek ister misiniz? e/h >> ")
            ...
            if ques.lower() == "e":
                ...
                print("""\n
                1)Uygulamaya Başla
                2)Bu Uygulamanın Öncekinden ne Farkı var?
                """)
                ur = int(input("\nSeçimini Gir >> "))
                ...
                if ur == 1:
                    ...
                    text_to_qr = input("QR Koduna yazılacak veriyi/yazıyı girin >> ")
                    time.sleep(2)
                    subprocess.run(["clear"])

                    print(f"Dönüştürülecek Veri >> {text_to_qr}")
                    time.sleep(2)

                    subprocess.run("clear")
                    subprocess.run(["qr",text_to_qr])
                    print(f"\nQR Kod oluşturuldu, QR'a yazdırılan veri >> {text_to_qr}")

                elif ur == 2:
                    subprocess.run(["clear"])
                    subprocess.run(["figlet","QR_CODE"])
                    print("\nBu uygulamanın öncekinden farkı QR kodunu terminale yazdırmasıdır.")
                    asd = input("\nUygulamaya geri dönmek ister misiniz? e/h >> ")
                    if asd.lower() == "e":
                        print("Yönlendiriliyorsunuz...")
                        time.sleep(2)
                        qr_term()
    ...
    print(
        """
        1)netdiscover                       18)IP_INFORMATION                              35)Canın mı sıkıldı? Buraya gir ;)
        2)nikto                             19)YouTube video indirici                      99)Çıkış
        3)msfvenom(%100 fud değil)          20)Cryptography
        4)rootkit scanner                   21)SpeedTest
        5)weevely web shell                 22)png to jpg
        6)skipfish                          23)domain check
        7)nmap                              24)Instagram id,pp,data
        8)sqlmap                            25)mac changer
        9)telefon osint                     26)URL kısaltıcı
        10)dnsmap                           27)GUI'li video indirici
        11)dirb                             28)Parola Türetici
        12)theHarvester                     29)QR Code üretici
        13)wafw00f                          30)Hexadecimal To Decimal Dönüştürücü
        14)dig                              31)Steghide
        15)wiki-search                      32)Çeviri
        16)insta_explore                    33)Covid Info
        17)url scraper                      34)Terminal QR Kod
        """
    )
    secim = int(input("Seçiminizi Girin : "))
